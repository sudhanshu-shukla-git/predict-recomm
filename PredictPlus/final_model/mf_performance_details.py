...
class mf_performance():
    ...
    def performance_based_recommendation(self,mf_category=None,mf_sub_category=None,risk=None,top_n=5,load_cache=True):
        try:
            return self.get_response(mutual_fund_performance(mf_category,mf_sub_category,risk,top_n,load_cache))
        except Exception as ex:
            return self.get_exception_response("Error occured, please try again. Exception-"+ex)                        
    
#     def get_mf_risk(self,scheme_name,risk=None,default="Moderate",):
#         selected_mf=None
#         selected_ratio=-1
#         mf_df=self.mf_df.copy(deep=True)
        
#         if risk is not None:
#             mf_df=self.mf_df[self.mf_df["Risk"]==risk]
            
#         mf_risk_score= mf_df[mf_df["fund_name"].str.lower().str.strip()==str(scheme_name).strip().lower()]["Risk"].to_list()
...
#                     selected_mf=mf 
#             mf_risk_score= list(set(mf_df[mf_df["fund_name"].str.lower()==str(selected_mf).lower()]["Risk"]))[0]
#             return str(mf_risk_score)
#         else:
#             return str(mf_risk_score)
    def get_mf_risk(self,scheme_name,default="Moderate"):
        fund_risk=pd.read_pickle("fund_risk_score.pkl.bz2",compression="bz2")
        risk=fund_risk[fund_risk["fund_name"].str.lower().str.strip()==scheme_name.lower().strip()]["risk"]
        
        if len(risk)>1:
            risk= risk[0]
        elif (len(risk)==0) or (risk is None):
            risk= default
